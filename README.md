...
| 테이블명                | 설명                     |
| ------------------     | ---------------------- |
| `users`                | 회원 정보, 권한, 상태(탈퇴/활성), 포인트 등  |
| `request`              | 요청글 (공식/비공식 장소, 카테고리, 세부 항목 포함) |
| `status_logs`          | 요청에 대한 응답글, 자발적 공유, 공지 등을 포함  |
| `status_log_images`    | 상태 로그 이미지 (1:N 관계)  |
| `places`               | 공식 장소 데이터 (위치, 승인, 반려 등) |
| `allowed_request_types`| 장소별 요청 가능 카테고리 |
| `favorite_places`      | 사용자 즐겨찾기 장소 |
...
> 각 테이블은 JPA 기준 `@OneToMany`, `@ManyToOne` 관계로 매핑되어 있습니다.

* **1\:N** : `users` ↔ `request`, `status_logs`, `points`, `deleted_user_log`, `admin_action_log`, `favorite_places`
* **1\:N** : `places` ↔ `status_logs`, `request`, `reports`, `allowed_request_types`, `favorite_places`
* **1\:N** : `request` ↔ `status_logs`
* **1\:N** : `status_logs` ↔ `status_log_images`
...
> RealCheck 프로젝트는 `.env` 파일을 기반으로 외부 설정 값을 주입받으며, `application.properties`가 아래와 같이 구성되어 있습니다.
...
<br>
 ...
 #### **🔗 서비스 주소: [https://real-check.store](https://real-check.store)**

#### 🔹 환경 설명

- HTTPS: Nginx Reverse Proxy + Certbot
- 배포 구조: EC2 (Ubuntu) + Spring Boot + RDS + Redis
- 도메인 연결: Cafe24 → EC2 퍼블릭 IP (A 레코드)
...
| 구성 요소      | 역할 및 설명                                      |
| -------------- | ------------------------------------------------- |
| **사용자 브라우저** | HTTPS 요청 시작 (도메인 real-check.store)     |
| **Nginx**         | 리버스 프록시 역할 + HTTPS 처리 (`443 → 8080`) |
| **Certbot**       | 무료 SSL 인증서 발급 + 자동 갱신 (`cron`)      |
| **Spring Boot (EC2)** | 메인 애플리케이션 실행 (`8080 포트`)      |
| **RDS (MySQL)**   | 사용자, 요청, 답변, 포인트 등 데이터 저장소     |
| **Redis (로컬)**  | 뷰 카운트, 세션, 중복 방지 등 캐시 용도        |
| **Cafe24 도메인** | A 레코드로 EC2 퍼블릭 IP에 연결                |
